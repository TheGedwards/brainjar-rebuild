    ...
    <>
      <section className="px-6 py-16 text-center sm:py-20">
        <Frame>
          ...
          <h1 className="display mt-4 text-[30px] leading-tight sm:text-[44px]">{s.name}</h1>
          <Lozenge className="my-6" />
          <p className="mx-auto max-w-xl text-lg italic leading-relaxed text-ink-soft">{s.lede}</p>
            ...
            <div className="mt-14 grid gap-6 sm:grid-cols-3">
              {s.subs.map((sub, i) => (
                ...
                >
                  ...
                  <h3 className="display mt-3 text-sm tracking-[0.18em] group-hover:text-tincture">
                    {sub.name}
                  </h3>
                  <p className="mt-3 text-[15px] italic leading-relaxed text-ink-soft">{sub.blurb}</p>
                </Link>
              ))}
            </div>
          </div>
            ...
            <div className="mt-12 grid gap-6 sm:grid-cols-3">
              {related.map((p) => (
                ...
                >
                  ...
                  {p.tagline && <div className="mt-2 text-[15px] italic text-ink-soft">{p.tagline}</div>}
                </Link>
              ))}
            </div>
          </div>
